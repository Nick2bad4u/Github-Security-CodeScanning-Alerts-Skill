...
assert(
    pkg.name === "github-manage-security-alerts-skill",
    "package name must be github-manage-security-alerts-skill"
);
assert(
    pkg.repository?.url ===
...
assert(
    /^metadata:/m.test(skillMd),
    "SKILL.md frontmatter must include metadata"
);
